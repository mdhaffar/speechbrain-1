#!/usr/bin/env python3
"""Recipe for training a TiCodec Neural Audio Codec.
For more details about TiCodec
...
class HifiGanBrain(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        calculcates the scores and the features of the discriminator
        ...
        z, global_features = self.modules.generator(y) #[:, :, : y.size(2)]
        q, loss_q, local_token, g, global_style_token = self.modules.quantizer(z, global_features)
      
        dec = self.modules.decoder(q,g)
    ...
    def compute_objectives(self, batch, stage):
        ...
        dec = self.compute_forward(batch, sb.core.Stage.TRAIN)
        
        mel_synthetic = hparams["mel_spectogram"](audio=dec.squeeze(0))

        #Mel for Coarse frequency resolution (n_fft = 512, hop = 120 and win = 512)
        mel_synthetic_CFR = hparams["mel_spectogram"](audio=dec.squeeze(0), n_fft=512, hop_length=120, win_length=512)
        mel_real_CFR = hparams["mel_spectogram"](audio=y.squeeze(0), n_fft=512, hop_length=120, win_length=512)

        #Mel for Medium resolution (n_fft = 256, hop = 60 and win = 256)
        mel_synthetic_MR = hparams["mel_spectogram"](audio=dec.squeeze(0), n_fft=256, hop_length=60, win_length=256)
        mel_real_MR = hparams["mel_spectogram"](audio=y.squeeze(0), n_fft=256, hop_length=60, win_length=256)

        #Mel for Fine temporal resolution (n_fft = 128, hop = 30 and win = 128)
        mel_synthetic_FTR = hparams["mel_spectogram"](audio=dec.squeeze(0), n_fft=128, hop_length=30, win_length=128)
        mel_real_FTR = hparams["mel_spectogram"](audio=y.squeeze(0), n_fft=128, hop_length=30, win_length=128)

        #MPD
        mpd_y, _ = self.modules.mpd(y)
        mpd_synthetic, _ = self.modules.mpd(dec.detach())
        loss_mpd, r_losses_mpd, g_losses_mpd = self.hparams.discriminator_loss(mpd_y, mpd_synthetic)
        print(loss_mpd, flush=True)

        #MSD
        msd_y, _ = self.modules.msd(y)
        msd_synthetic, _ = self.modules.msd(dec.detach())
        loss_msd, r_losses_msd, g_losses_mpd = self.hparams.discriminator_loss(msd_y, msd_synthetic)
    
        #MSTFTD
        mstftd_y, _ = self.modules.mstftd(y)
        mstftd_synthetic, _ = self.modules.mstftd(dec.detach())
        loss_mstftd, r_losses_mstftd, g_losses_mstftd = self.hparams.discriminator_loss(mstftd_y, mstftd_synthetic)

        loss_all_disc = loss_mpd + loss_msd + loss_mstftd
        print(loss_all_disc)

        return loss_all_disc
    ...
    def fit_batch(self, batch):
        ...
        loss = self.compute_objectives(batch, sb.core.Stage.TRAIN)
...
def dataio_prepare(hparams):
    ...
    def audio_pipeline(wav):
        wav = wav.replace('/data3/renyong/Dataset/LibriTTS/LibriTTS/','/data/coros3/smdhaffar/TiCodec/LibriTTS/')
    ...
    def audio_pipeline(wav):
        # Fix path
        wav = wav.replace(
            '/data3/renyong/Dataset/LibriTTS/LibriTTS/',
            '/data/coros3/smdhaffar/TiCodec/LibriTTS/'
        )

        # Read waveform
        audio = sb.dataio.dataio.read_audio(wav)  # 1D array
        audio = torch.FloatTensor(audio)          # [T]
        audio = audio.unsqueeze(0).unsqueeze(0)   # [1, 1, T] ✅ add batch & channel dims

        # Pad to fixed size (segment_size from hparams)
        audio = torch.nn.functional.pad(
            audio, (0, segment_size - audio.size(-1)), "constant"
        )  # final shape: [1, 1, segment_size]

        # Mel spectrogram (expected for loss or auxiliary task)
        mel = hparams["mel_spectogram"](audio=audio.squeeze(0))  # → [80, T_mel]

        return mel, audio.squeeze(0)  # mel: [80, T], sig: [1, T]
